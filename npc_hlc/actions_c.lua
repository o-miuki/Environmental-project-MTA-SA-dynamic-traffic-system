...
function makeNPCDriveToPos(npc,x,y,z)
	...
	-- Check for reverse mode flag (set by deadlock handler)
	local reverseMode = getElementData(npc, "npc.reverse_mode")
	if reverseMode then
		-- In reverse mode: stop accelerating, apply brake/reverse, steer slightly
		setPedControlState(npc, "accelerate", false)
		setPedControlState(npc, "brake_reverse", true)
		-- Slight random steering while reversing to avoid getting stuck
		local steerDir = (getTickCount() % 2000) < 1000
		setPedControlState(npc, "vehicle_left", steerDir)
		setPedControlState(npc, "vehicle_right", not steerDir)
		return
	end
...
-- =============================================================================
-- HONKING SYSTEM - Process horn control for frustrated NPCs
-- =============================================================================
local hornCache = {}

function updateNPCHorns()
	for _, veh in ipairs(getElementsByType("vehicle")) do
		local driver = getVehicleController(veh)
		if driver and isElement(driver) and getElementType(driver) ~= "player" then
			local shouldHonk = getElementData(driver, "npc.honking")
			local currentHonk = hornCache[driver]
			
			-- Only update if state changed
			if shouldHonk ~= currentHonk then
				hornCache[driver] = shouldHonk
				setPedControlState(driver, "horn", shouldHonk or false)
			end
		end
	end
end

-- Update horn states periodically (not every frame to save performance)
setTimer(updateNPCHorns, 100, 0)

-- Cleanup on element destroy
addEventHandler("onClientElementDestroy", root, function()
	if getElementType(source) == "ped" then
		hornCache[source] = nil
	end
end)
